...
export const CliProcessamentoDiagnosticoMessages = {
  ...
  resumoExecutivoAcaoSugerida: 'Suggested action: filter by file with --include "<模式>" or use --export for offline analysis',
  // Basic summary (console)
  principaisTiposTitulo: '  Messages  Main types:',
  principaisTiposLinha: (tipo: string, count: number) => `     • ${tipo}: ${count}`,
  topArquivosTitulo: `  ${ICONES_ARQUIVO.diretorio} Top 文件:`,
  ...
  // Archetypes
  arquetiposDetectados: (qtd: number) => `检测到 ${qtd} archetype candidates`,
  ...
  driftDetectado: (anterior: string | undefined, atual: string | undefined) => `Drift 检测到: ${anterior} → ${atual}`,
  arquetipoMantido: (atual: string | undefined) => `Archetype kept: ${atual}`,
  novosArquivosRaiz: (joined: string) => `New 文件 in root: ${joined}`,
  ...
  relatorioFullFragmentado: (manifestFile: string | undefined) => `Fragmented full 报告. Manifest: ${manifestFile}`,
  relatorioScanSalvo: (dir: string) => `Scan 报告 saved in ${dir}`,
  falhaExportarRelatorioScanOnly: (errMsg: string) => `Failed to export scan-only 报告: ${errMsg}`,
  autoFixModoAgressivo: 'Aggressive mode activated - applying more fixes (use with caution)',
  falhaGerarRelatorioMarkdownMetadados: (errMsg: string) => `Failed to generate Markdown 报告 with metadata: ${errMsg}`,
  relatoriosExportadosPara: (dir: string) => `报告s exported to ${dir}`,
  falhaSalvarRelatorioJson: (errMsg: string) => `Failed to save JSON 报告: ${errMsg}`,
  falhaExportarRelatorios: (errMsg: string) => `Failed to export 报告s: ${errMsg}`,
  erroFatalDiagnostico: (errMsg: string) => `Fatal 错误 during diagnosis: ${errMsg}`
} as const;
