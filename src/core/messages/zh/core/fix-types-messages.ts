...
const MENSAGENS_PROGRESSO = {
  processandoArquivos: (count: number) => `${ICONES_ARQUIVO.diretorio} Processing ${count} 文件...`,
...
export const MENSAGENS_SUCESSO = {
  nenhumTipoInseguro: `${ICONES_STATUS.ok} No unsafe types 检测到! Code has good type safety.`,
...
export const MENSAGENS_CLI_CORRECAO_TIPOS = {
  linhaEmBranco: '',
  erroExecutar: (mensagem: string) => `错误 running fix-types: ${mensagem}`,
  ...
  exportandoRelatorios: `${ICONES_ACAO.export} 导出ing reports...`,
  // Verbose / detailed logs
  verboseAnyDetectado: (arquivo: string, linha: string) => `  ${ICONES_TIPOS.any} ${arquivo}:${linha} - any 检测到 (fix recommended)`,
  verboseAsAnyCritico: (arquivo: string, linha: string) => `  ${ICONES_TIPOS.corrigir} ${arquivo}:${linha} - "as any" 检测到 (CRITICAL - fix required)`,
  verboseAngleAnyCritico: (arquivo: string, linha: string) => `  ${ICONES_TIPOS.corrigir} ${arquivo}:${linha} - "<any>" 检测到 (CRITICAL - legacy syntax)`,
  ...
  correcoesResumoSucesso: (qtd: number) => `${ICONES_STATUS.ok} ${qtd} 文件(s) fixed`,
  ...
  correcoesResumoFalhas: (qtd: number) => `${ICONES_STATUS.falha} ${qtd} file(s) with 错误`,
