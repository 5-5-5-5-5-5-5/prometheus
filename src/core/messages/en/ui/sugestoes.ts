...
export const SUGESTOES_COMANDOS = {
  usarFull: `${ICONES.feedback.dica} Use --full for a detailed report with all information`,
  ...
  usarAutoFix: `${ICONES.feedback.dica} Use --auto-fix to apply automatic fixes`,
} as const;
...
export const SUGESTOES_DIAGNOSTICO = {
  ...
  projetoGrande: [
    ...
    `${ICONES.feedback.dica} Use --quick for a fast initial analysis`,
  ],
...
export const SUGESTOES_AUTOFIX = {
  autoFixDisponivel: `${ICONES.feedback.dica} Automatic fixes available - use --auto-fix`,
  ...
  validarDepois: [
    ...
    `${ICONES.feedback.dica} Run npm run build to check if the code compiles`,
...
export const SUGESTOES_PODAR = {
  cuidado: [
    `${ICONES.feedback.atencao} Pruning permanently removes files!`,
    `${ICONES.feedback.importante} Make sure you have backup or version control`,
  ],
  revisar: `${ICONES.feedback.dica} Review the file list before confirming`,
