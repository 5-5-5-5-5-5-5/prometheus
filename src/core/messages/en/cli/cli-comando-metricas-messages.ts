...
export const CliComandoMetricasMessages = {
  ...
  executionLine: (timestampISO: string, totalArquivos: number, duracaoAnalise: string, duracaoParsing: string, cacheHits: number, cacheMiss: number) => `- ${timestampISO} | files=${totalArquivos} analysis=${duracaoAnalise} parsing=${duracaoParsing} cache(h/m)=${cacheHits}/${cacheMiss}`,
  ...
  averages: (mediaAnalise: string, mediaParsing: string) => `\nAverages: analysis=${mediaAnalise} parsing=${mediaParsing}`
} as const;
