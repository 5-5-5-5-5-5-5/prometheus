...
export const MENSAGENS_AUTOFIX = {
  ...
  concluido: (aplicadas: number, falhas: number) => `${ICONES.nivel.sucesso} Auto-fix completed: ${aplicadas} applied, ${falhas} failed`,
  naoDisponivel: `${ICONES.feedback.info} No automatic fix available`,
  // Flags and modes
  flags: {
    fixSafe: `${ICONES.comando.guardian} Flag --fix-safe detected: enabling conservative mode`,
    requireMutateFS: `${ICONES.status.falha} Auto-fix unavailable at this time.`
  },
  // Progress logs
  logs: {
  ...
  dicas: {
    ...
    executarBuild: `${ICONES.feedback.dica} Run \`npm run build\` to verify the code compiles`,
