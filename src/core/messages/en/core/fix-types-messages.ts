...
const MENSAGENS_INICIO = {
  ...
  modo: (dryRun: boolean) => `${dryRun ? ICONES_ACAO.analise : ICONES_ACAO.correcao} Mode: ${dryRun ? 'Analysis (dry-run)' : 'Apply fixes'}`
} as const;
...
export const MENSAGENS_CLI_CORRECAO_TIPOS = {
  linhaEmBranco: '',
  erroExecutar: (mensagem: string) => `Error running fix-types: ${mensagem}`,
  ...
  analiseDetalhadaSalva: `${ICONES_ARQUIVO.arquivo} Detailed analysis saved at: .prometheus/fix-types-analise.json`,
  ...
  incertosTitulo: (qtd: number) => `${ICONES_FEEDBACK.pergunta} ${qtd} cases with uncertain analysis (<70% confidence):`,
  ...
  correcoesResumoSucesso: (qtd: number) => `${ICONES_STATUS.ok} ${qtd} file(s) fixed`,
  ...
  correcoesResumoFalhas: (qtd: number) => `${ICONES_STATUS.falha} ${qtd} file(s) with error`,
