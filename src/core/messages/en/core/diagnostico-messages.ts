...
export const MENSAGENS_INICIO: Record<ModoOperacao, string> = {
  ...
  executive: `${ICONES_DIAGNOSTICO.executive} Executive analysis (criticals only)`,
  quick: `${ICONES_DIAGNOSTICO.rapido} Quick analysis`
};
...
export const MENSAGENS_PROGRESSO = {
  varredura: (total: number) => `${ICONES_DIAGNOSTICO.arquivos} Scanning ${total} file${total !== 1 ? 's' : ''}...`,
  ...
  export: (formato: string) => `${ICONES_DIAGNOSTICO.export} Exporting report (${formato})...`
} as const;
...
export const MENSAGENS_CONCLUSAO = {
  sucesso: (ocorrencias: number) => `${ICONES_DIAGNOSTICO.sucesso} Diagnosis completed: ${ocorrencias} occurrence${ocorrencias !== 1 ? 's' : ''} found`,
  semProblemas: `${ICONES_DIAGNOSTICO.sucesso} No problems found! Code is in great shape.`,
  exportado: (caminho: string) => `${ICONES_DIAGNOSTICO.export} Report saved at: ${caminho}`
} as const;
...
export const MENSAGENS_ERRO = {
  falhaAnalise: (erro: string) => `${ICONES_DIAGNOSTICO.erro} Analysis failed: ${erro}`,
  falhaExport: (erro: string) => `${ICONES_DIAGNOSTICO.erro} Export failed: ${erro}`,
  falhaGuardian: (erro: string) => `${ICONES_DIAGNOSTICO.erro} Guardian failed: ${erro}`,
  falhaAutoFix: (erro: string) => `${ICONES_DIAGNOSTICO.erro} Auto-fix failed: ${erro}`,
...
export const MENSAGENS_FILTROS = {
  ...
  exclude: (patterns: string[]) => `Exclude: ${patterns.length > 0 ? patterns.join(', ') : 'default patterns'}`,
...
export const MENSAGENS_ESTATISTICAS = {
  ...
  arquivos: (total: number) => `Files analyzed: ${total}`,
