...
export function detectarContextoProjeto(opcoes: DetectarContextoOpcoes): ContextoProjeto {
  ...
  const isTestPath = /(^|\/)tests?(\/|\.)/i.test(rel) || /\.(test|spec)\.(ts|js|tsx|jsx)$/i.test(p) || /__tests__/.test(rel);
  const belongsToTestExt = /\.(ts|js|tsx|jsx)$/i.test(p);
  const hasTestContent = belongsToTestExt && (/^(describe|it|test|expect)\s*\(/m.test(conteudo) || /\.(it|test|describe|expect)\s*\(/m.test(conteudo));
  contexto.isTest = isTestPath || hasTestContent;
...
export function sugerirFrameworks(contexto: ContextoProjeto): string[] {
  ...
  /**
  * Verifica se um caminho de arquivo pertence ao contexto de documentação.
  */
  export function isContextoDocumentacao(relPath: string): boolean {
